export function collisionDetection({ player, obstacle }) {
    if (
        (player.position.x + player.aspectRatio.w) >= (obstacle.positionPipeDown.x) &&
        (player.position.x + player.aspectRatio.w) <= (obstacle.positionPipeDown.x + obstacle.aspectRatio.w) &&
        (
